 ...
 */
function kaspweb_enqueue_fonts() {
	$default = array(
		'default',
		'Default',
		'arial',
	...
	foreach ( $fonts as $font ) {
		if ( ! in_array( $font, $default, true ) ) {
			kaspweb_enqueue_google_fonts( $font );
		}
	}
}
add_action( 'wp_enqueue_scripts', 'kaspweb_enqueue_fonts' );
 ...
 */
function kaspweb_enqueue_google_fonts( $font ) {
	$font = explode( ',', $font );
	$font = $font[0];
	...
	// CSS ENQUEUE.
	wp_enqueue_style( 'kaspweb-google-$font', 'https://fonts.googleapis.com/css?family=$font', false, null, 'all' );
}