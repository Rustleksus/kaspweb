wp.customize.controlConstructor['kaspweb-range'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			input_default,
			changeAction,
			kaspweb_range_input_number_timeout;

		// Update the text value
		jQuery( 'input[type=range]' ).on( 'mousedown', function() {

			range 			= jQuery( this );
			range_input 	= range.parent().children( '.kaspweb-range-input' );
		...
		function kaspweb_autocorrect_range_input_number( input_number, timeout ) {
			...
			clearTimeout( kaspweb_range_input_number_timeout );

			kaspweb_range_input_number_timeout = setTimeout( function() {
		...
		jQuery( 'input.kaspweb-range-input' ).on( 'change keyup', function() {

			kaspweb_autocorrect_range_input_number( jQuery( this ), 1000);

		} ).on( 'focusout', function() {

			kaspweb_autocorrect_range_input_number( jQuery( this ), 0);

		} );

		// Handle the reset button
		jQuery( '.kaspweb-reset-slider' ).on('click', function() {
