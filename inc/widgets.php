 ...
 */
add_filter( 'dynamic_sidebar_params', 'kaspweb_widget_classes' );

if ( ! function_exists( 'kaspweb_widget_classes' ) ) {
	 ...
	 * @return array $params
	 */
	function kaspweb_widget_classes( $params ) {

		global $sidebars_widgets;

		/*
		 * When the corresponding filter is evaluated on the front end
		 * Когда соответствующий фильтр вычисляется на переднем конце
		 * this takes into account that there might have been made other changes.
		 * при этом учитывается, что могли быть внесены и другие изменения.
		 */
        ...
        // Применяйте изменения только в том случае, если установлен идентификатор боковой панели и классы виджета зависят от количества виджетов в боковой панели.
		if ( isset( $params[0]['id'] ) && strpos( $params[0]['before_widget'], 'dynamic-classes' ) ) {
			...
			if ( 0 === $widget_count % 4 || $widget_count > 6 ) {
                ...
                // Четыре виджета в строке, если их ровно четыре или больше шести.
...
} // End of if function_exists( 'kaspweb_widget_classes' ).