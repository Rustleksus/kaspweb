...
// Query args
$args = array(
	'posts_per_page' => apply_filters( 'ocean_related_blog_posts_count', absint( get_theme_mod( 'ocean_blog_related_count', '2' ) ) ),
	'orderby'        => 'rand',
...
$kaspweb_related_query = new WP_Query( $args );

// If the custom query returns post display related posts section
if ( $kaspweb_related_query->have_posts() ) :
	...
	<section id="related-posts" class="<?php echo esc_attr( $classes ); ?>">
        ...
        <!-- FA  <div class="oceanwp-row clr">-->
		<div class="oceanwp-row clr row"> 

			<?php $oceanwp_count = 0; ?>

			<?php foreach( $kaspweb_related_query->posts as $post ) : setup_postdata( $post ); ?>
				...
				$classes[]	= kaspweb_grid_class( $oceanwp_columns );
					...
					elseif ( has_post_thumbnail() ) : ?>
						...
						<figure class="related-post-media clr">
							...
							<a href="<?php the_permalink(); ?>" class="related-thumb">
								...
								if ( function_exists( 'kaspweb_express_image_attributes' ) ) {
									$img_atts 	= kaspweb_express_image_attributes( $img_url[1], $img_url[2], $img_width, $img_height );
								}

								// If Ocean Extra is active and has a custom size
								if ( function_exists( 'kaspweb_express_resize' )
									&& ! empty( $img_atts ) ) { ?>

									<img src="<?php echo kaspweb_express_resize( $img_url[0], $img_atts[ 'width' ], $img_atts[ 'height' ], $img_atts[ 'crop' ], true, $img_atts[ 'upscale' ] ); ?>" alt="<?php the_title_attribute(); ?>" width="<?php echo esc_attr( $img_width ); ?>" height="<?php echo esc_attr( $img_height ); ?>"<?php kaspweb_schema_markup( 'image' ); ?> />
